...
#if defined(NDEBUG)
# error "Kazukoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd12be38f2db9052970fa66dd888d70a0477af7b731b6d814126708e53de6d2ea");
...
const string strMessageMagic = "Kazukoin Signed Message:\n";
...
{
    // Kazukoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kazukoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kazukoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f822f3831009ed384415043b66f8b2afa393e177e385a7c72cc70686fabbd677277491c3a81242abe751c758d02484eacc76c497b2ac0e8e6ddda974169309c7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524302748;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084838418;

        if (fTestNet)
        {
            block.nTime    = 1524302730;
        ...
        assert(block.hashMerkleRoot == uint256("0x07c9c10634722e6c34634879a011fb05cff664b87b82b9c3d723bd99a9fa85d2"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KazukoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KazukoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KazukoinMiner(CWallet *pwallet)
{
    printf("KazukoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kazukoin-miner");
    ...
    try { loop {
        ...
        printf("Running KazukoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KazukoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KazukoinMiner, pwallet));
}
